...
type EbetData struct {
	...
	WithHoldingTotal       interface{} `db:"with_holding_total"`
	WithHoldingDetail      interface{} `db:"with_holding_detail"`
	PayoutWithoutholding   interface{} `db:"payout_withoutholding"`
}

func CopyEbet(timeEnd time.Time) {
	...
	insertSQL := `INSERT INTO ebet_wager (
        bet_history_id,
        game_type,
        bet_map,
        judge_result,
	...
	selectSQL := `SELECT * FROM ebet_wager LIMIT 1000`

	var original []EbetData
	...
	for i := 1; i <= 31; i++ {
		fmt.Println("Inserting Ebet for day", i)
