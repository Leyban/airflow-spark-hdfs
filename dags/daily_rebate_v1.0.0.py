...
def init_identity_member_table():
    ...
    create_identity_db_sql = f"CREATE DATABASE IF NOT EXISTS identity LOCATION '{HIVE_WAREHOUSE_DIR}'"
...
def insert_into_hive(new_member_df):
    ...
    query = f"""INSERT INTO identity.member (
        id,
        uuid,
        login_name,
        full_name,
...
def get_allbet_wager():
    ...
    today = datetime.utcnow().replace(hour=UTC_EXEC_TIME, minute=0, second=0, microsecond=0)
