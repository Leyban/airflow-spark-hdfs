...
def update_members_on_sqlite(missing_member_df):
    import pandas as pd
...
def get_allbet_wager(date_from, date_to):
...
def get_asiagaming_wager(date_from, date_to):
    conn_collector_pg_hook = PostgresHook(postgres_conn_id='collector_conn_id')
...
def get_agslot_wager(date_from, date_to):
    conn_collector_pg_hook = PostgresHook(postgres_conn_id='collector_conn_id')
...
def get_agyoplay_wager(date_from, date_to):
    conn_collector_pg_hook = PostgresHook(postgres_conn_id='collector_conn_id')
...
def get_sagaming_wager(date_from, date_to):
    conn_collector_pg_hook = PostgresHook(postgres_conn_id='collector_conn_id')
...
def get_simpleplay_wager(date_from, date_to):
    conn_collector_pg_hook = PostgresHook(postgres_conn_id='collector_conn_id')
...
def get_simpleplayfisher_wager(date_from, date_to):
    conn_collector_pg_hook = PostgresHook(postgres_conn_id='collector_conn_id')
...
def get_pgsoft_wager(date_from, date_to):
    conn_collector_pg_hook = PostgresHook(postgres_conn_id='collector_conn_id')
...
def get_ebet_wager(date_from, date_to):
    conn_collector_pg_hook = PostgresHook(postgres_conn_id='collector_conn_id')
...
def get_bti_wager(date_from, date_to):
    conn_collector_pg_hook = PostgresHook(postgres_conn_id='collector_conn_id')
...
def get_sabacv_wager(date_from, date_to):
    conn_collector_pg_hook = PostgresHook(postgres_conn_id='collector_conn_id')
...
def get_saba_wager(date_from, date_to):
    conn_collector_pg_hook = PostgresHook(postgres_conn_id='collector_conn_id')
...
def get_saba_number(date_from, date_to):
    conn_collector_pg_hook = PostgresHook(postgres_conn_id='collector_conn_id')
...
def get_saba_virtual(date_from, date_to):
    conn_collector_pg_hook = PostgresHook(postgres_conn_id='collector_conn_id')
...
def get_tfgaming_wager(date_from, date_to):
    conn_collector_pg_hook = PostgresHook(postgres_conn_id='collector_conn_id')
...
def get_evolution_wager(date_from, date_to):
    conn_collector_pg_hook = PostgresHook(postgres_conn_id='collector_conn_id')
...
def get_genesis_wager(date_from, date_to):
    conn_collector_pg_hook = PostgresHook(postgres_conn_id='collector_conn_id')
...
def get_weworld_wager(date_from, date_to):
    conn_collector_pg_hook = PostgresHook(postgres_conn_id='collector_conn_id')
...
def get_digitain_wager(date_from, date_to):
    conn_collector_pg_hook = PostgresHook(postgres_conn_id='collector_conn_id')
...
def get_reward_point(x, level_setting_df):
    reward_df = level_setting_df[(level_setting_df['level'] == x['level']) & (level_setting_df['currency'] == x['currency'])]

    if reward_df.shape[0] == 0:
        print("Currency not supported: ", x['currency'])
        return 0

    reward_point = reward_df.iloc[0]['reward_point']
    reward_point_percent = reward_df.iloc[0]['reward_point_percent']

    total_reward_point = reward_point + (reward_point_percent * 0.01 * x['eligible_stake_amount'])

    return total_reward_point


def drop_members_claimed_via_bonus(wager_df):
    conn_reward_pg_hook = PostgresHook(postgres_conn_id='reward_conn_id')
    ...
    bonus_df = conn_reward_pg_hook.get_pandas_df(rawsql)
...
def process_wagers_rebate(wager_df, product, **context):
    ...
    wager_df = drop_members_claimed_via_bonus(wager_df)
    ...
    clear_old_data_sql = f"DELETE FROM {DAILY_REBATE_TABLE} WHERE product = '{product}' AND rebate_date = '{execution_date}' AND status = {REBATE_STATUS_PROCESSING}"
    conn_reward_pg_hook.run(clear_old_data_sql)

    wager_df.to_sql(DAILY_REBATE_TABLE, con=engine_payment, if_exists='append', index=False)
...
def process_wagers_reward(wager_df, product, **context):
    ...
    wager_df['reward_point'] = wager_df.apply(lambda x: get_reward_point(x, level_df), axis=1)
    ...
    clear_old_data_sql = f"DELETE FROM {DAILY_REWARD_TABLE} WHERE product = '{product}' AND reward_date = '{execution_date}'"
    conn_reward_pg_hook.run(clear_old_data_sql)

    wager_df.to_sql(DAILY_REWARD_TABLE, con=engine_payment, if_exists='append', index=False)
...
def process_wagers_vip(wager_df, product, **context):
    ...
    clear_old_data_sql = f"DELETE FROM {DAILY_VIP_TABLE} WHERE product = '{product}' AND vip_date = '{execution_date}'"
    conn_reward_pg_hook.run(clear_old_data_sql)

    wager_df.to_sql(DAILY_VIP_TABLE, con=engine_payment, if_exists='append', index=False)
