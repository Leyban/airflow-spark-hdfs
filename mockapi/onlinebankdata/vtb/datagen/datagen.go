...
func createDummyOnlineBankData(tTo time.Time, hoursAgo int) error {
	db, err := sqlx.Connect("postgres", "user=postgres password=secret dbname=dummyDB sslmode=disable")
	if err != nil {
		log.Fatal(err)
	}
	...
	randTime := tTo.Add(-time.Hour * time.Duration(RandNumber(hoursAgo-1)))
...
func createDeposits(percent, duplicates int, tTo time.Time, hoursAgo int) error {
	dummydb, err := sqlx.Connect("postgres", "user=postgres password=secret dbname=dummyDB sslmode=disable")
	if err != nil {
		log.Fatal(err)
	}

	timeSpan := tTo.Add(-time.Hour * time.Duration(hoursAgo))
...
func GenerateData(dummySize int, dateFrom, dateTo string) error {

	tFrom, err := time.Parse("1/2/2006", dateFrom)
	if err != nil {
		return err
	}
	tTo, err := time.Parse("1/2/2006", dateTo)
	if err != nil {
		return err
	}

	tTo = tTo.AddDate(0, 0, 1).Add(-time.Second)

	tSpan := tTo.Sub(tFrom)
	hoursAgo := int(tSpan.Hours())
	...
	for i := 0; i < dummySize; i++ {
		err = createDummyOnlineBankData(tTo, hoursAgo)
		if err != nil {
			return err
		}
	}

	err = createDeposits(85, 15, tTo, hoursAgo)
	if err != nil {
		log.Fatal(err.Error())
	}
