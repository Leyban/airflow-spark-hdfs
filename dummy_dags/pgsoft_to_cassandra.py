...
def fetch_pgsoft_wager(**context):
    ...
    day_begin = datetime.now(timezone.utc).replace(
    ...
    try:
        ...
        if not exists_df.empty:
            df["duplicate"] = df.apply(
                lambda row: (row["bet_id"] in exists_df["bet_id"].values)
                and (
                    row["player_name"]
                    not in exists_df.loc[
                        exists_df["bet_id"] == row["bet_id"], "player_name"
                    ].values
                ),
                axis=1,
            )
        ...
        inserted_df["year_month"] = inserted_df["bet_time"].apply(
            lambda x: get_year_month(x)
        )
        toc = time.perf_counter()
        ...
        toc = time.perf_counter()
        print(
            f"Time for inserting to pgsoft_by_member and pgsoft_by_date: {toc - tic:0.4f} seconds"
        )
        conn.shutdown()
...
def get_existing_data(df, conn):
    ...
    for id_range in ranges:
        range_df = df[df["bet_id_range"] == id_range]
        range_df = range_df.reset_index(drop=True)
...
download_pgsoft = PythonOperator(
    task_id="download_pgsoft", python_callable=fetch_pgsoft_wager, dag=dag
)
